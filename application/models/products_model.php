...
class Products_model extends CI_Model
{
	...
	function getAllSearch($arraySearch = array() , $start, $limit, $assets = '')
	{
		...
		if($assets != ''){
			...
			foreach ($fillterAssest as $key => $fillterAsses) {
				...
				//$this->db->where("(p.catID =  ".$arraySearch['category_id']." OR ct.parentID = ".$arraySearch['category_id']." OR ct1.parentID = ".$arraySearch['category_id'].")");
				$where .= "(p.catID =  ".$fillterAsses." OR ct.parentID = ".$fillterAsses." OR ct1.parentID = ".$fillterAsses." OR ct2.parentID = ".$fillterAsses.")";
			}
    ...
    */
    ...
    function getAllSearchHomeEve($arraySearch = array() , $start, $limit)
     {
        ...
        if(isset($arraySearch['keysearch']) && $arraySearch['keysearch'] != "")
        {
        	...
        	/* 
        	$wherelike = "(p.productName LIKE '%".$arraySearch['keysearch']."%' OR p.seo_name LIKE '%".$arraySearch['keysearch']."%' OR p.productCode LIKE '%".$arraySearch['keysearch']."%')";
        	$wherelike	= " MATCH(p.productName) AGAINST ('".$arraySearch["keysearch"]."')";
        	$this->db->where($wherelike);
        	$this->db->where("MATCH (field) AGAINST (".$arraySearch['keysearch'].")", NULL, FALSE);
        	*/
			...
			// add new
        	//$this->db->or_where("MATCH (p.productName) AGAINST ('".$arraySearch['keysearch']."')", NULL, FALSE)->or_where("MATCH (p.seo_name) AGAINST ('".$arraySearch['keysearch']."')", NULL, FALSE);
        }
    ...
    function getProdUsingCates($id, $limit, $start)
    {
    	...
    	$this->db->where("(p.catID =  ".$id." OR ct.parentID = ".$id." OR ct1.parentID = ".$id." OR ct2.parentID = ".$id.")");
    	$this->db->where('p.status', 1 );
    	$this->db->order_by("stt", "stt");
    	$this->db->limit($limit, $start);
    	$query = $this->db->get();
    	$sql= $this->db->last_query();
    	//echo $sql;
    ...
    */
    ...
    public function getRandProductlq($id)
    {
		...
		{
			$sql = "SELECT * FROM tbl_promotion WHERE status = 1";
			$query = $this->db->query($sql);
			if($query->num_rows() > 0){
				return $query->result_array();			
			} else {
