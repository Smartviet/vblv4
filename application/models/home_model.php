...
class Home_model extends CI_Model
{
	...
	{
		$return = $data = array();
		$sql = "SELECT catID, parentID, catName, icon, keymaket, productcate, imgAds, keypage FROM `tbl_categories` WHERE `status` = 1 AND parentID = 0  ORDER BY stt ASC";
		$query = $this->db->query($sql);
	...
	public function checkisUsername($username)
	{
	...
	// check login
	public function checkisPass($email, $pass)
	{
		$query = $this->db->get_where('tbl_member', array('userPass' => "$pass", 'email' => $email));
		if($query->num_rows() > 0) {
			return true;
		}else {
			return false;
	...
	{
		$query = $this->db->get_where('tbl_member', array('email' => "$username" ) );
		//echo $this->db->last_query();
		if($query->num_rows() > 0) {
			return true;
		}else {
	...
	public function addMail($user, $mail)
	{
		$sql = "INSERT INTO tbl_member (fullname,email) VALUES ('{$user}', '{$mail}')";
		$query =  $this->db->query($sql);
		if($query){
			return array(
	...
	{
		$sql = "SELECT * FROM tbl_padding WHERE type = 4 AND status = 1 ";
		$query = $this->db->query($sql);
		if($query) {
			return $query->row();
		} else {
