			...
			<div class="span12">
				...
				<form accept-charset="utf-8" method="post"  id="contact" action="<?php echo base_url('admincp/products/edit')."/".$productDetail->productID;?>">
					...
					<div class="block-fluid">
												...
												if($value1['parent'] == $id){ ?>
                                					...
                                					foreach ($getMenulv2 as $key => $value2) {
                                						if($value2['parent'] == $id1){ ?>
                                							...
                                							<?php
                                								$id2 =  $value2['id'];
                                								foreach ($getMenulv2 as $key => $value3) {
                                									if($value3['parent'] == $id2) {?>
                                										<option value="<?= $value3['id'] ?>" <?php if($productDetail->catID == $value3['id'] ){ echo 'selected="selected"';} ?> >------<?= $value3['name'] ?></option>
                                									<?php }
                                								}
                                							?>
                                						<?php }
						...
						<div class="row-form price">
							<div class="span2 price1l">Khối lượng</div>
							<div class="span2 price1">
								<input type="text" placeholder="Khối lượng sản phẩm"  value="<?= $productDetail->weight ?>"
						...
						<div class="row-form">
							...
							<div class="span3">
								<input  id="productImage" name="productImage" type="text" size="60" value="<?= $productDetail->productImage ?>" class="required" />
							</div>
							<div class="span2">
						...
						<div class="row-form">
							<div class="span1">Khuyến mãi: </div>
							<div class="span4">
								<?php $arrpromotions = array_filter(explode(',', $productDetail->allPromotion)); ?>
								<script type="text/javascript">
								$('document').ready(function(){
									<?php foreach ($arrpromotions as $arrpromotion): ?>
									$("#allPromotioncheck<?= $arrpromotion ?>").attr('checked', true );
									<?php endforeach; ?>
								});
								</script>
								<?php foreach ($allPromotions as $allPromotion) { ?>
									<input id="allPromotioncheck<?= $allPromotion['id']; ?>" type="checkbox" name="allPromotion[]" value="<?= $allPromotion['id']?>"><?= $allPromotion['name']?> <br>
								<?php }?>
							...
							<div class="span2">
								<input type="text" placeholder="Số thứ tự"  value="<?= $productDetail->stt ?>" name="stt" id="stt" />
							</div>
							<div class="clear"></div>
						</div>
